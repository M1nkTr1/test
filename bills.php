...
// tạo hoá đơn tháng mới
if(isset($_POST['generate'])){
    $query = "SELECT * from rooms";
    $rooms = mysqli_query($con, $query);
    $oneWeekLater = date("Y-m-d", strtotime(date("Y-m-d") . " +1 week"));

    while ($room = mysqli_fetch_array($rooms)) {
        $roomId = $room['id'];
        $oldWaterNumber = getClosestWaterNumber($roomId);
        $oldElectricityNumber = getClosetElectricityNumber($roomId);

        if ($roomId && $oldWaterNumber !== null && $oldElectricityNumber !== null) {
            $query = "INSERT INTO bills(roomId, oldWaterNumber, oldElectricityNumber, payDate) VALUES($roomId, $oldWaterNumber, $oldElectricityNumber, '$oneWeekLater')";
            mysqli_query($con, $query);
        }
            ...
            <div class="flex-1 p-4">
                <div>
                    ...
                    <form class="w-full pt-4" method="POST">
                        <button <?php echo $canCreate == true ? '' : 'disabled' ?> name="generate" type="submit" class="text-white bg-slate-800 p-3 w-full disabled:cursor-not-allowed disabled:bg-slate-400">Tạo hoá đơn tháng <?php echo date("m") ?></button>
                    </form>
                ...
                ?>
                    <div class="border border-gray-100 hover:bg-slate-100 duration-100 text-slate-500 p-4 text-sm relative">
                        ...
                        <form method="POST" class="p-4 flex items-center justify-center gap-4">
                            ...
                            ?>
                                <input type="hidden" name="ma" value="<?php echo $bill['id'] ?>">
                                <a href="bills_edit.php?ma=<?php echo $bill['id'] ?>" class="cursor-pointer material-symbols-outlined p-2 bg-slate-100 rounded-full hover:bg-slate-200 duration-100">
                                    edit
                                </a>
                            ...
                            ?>
                            <a target="_blank" href="bills_print.php?ma=<?php echo $bill['id'] ?>" class="cursor-pointer material-symbols-outlined p-2 bg-slate-100 rounded-full hover:bg-slate-200 duration-100">
                                print
                            </a>
                        </form>
                    </div>
