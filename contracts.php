                ...
                if (mysqli_num_rows($contracts) == 0) {
                    ...
                    while ($contract = mysqli_fetch_array($contracts)) {
                        ...
                        echo '<h1 class="mt-2">Trạng thái: <span class="' . ((new DateTime() > new DateTime($contract['endDate']) || $contract['cancelledAt']) ? 'text-rose-500' : '') . '">' . ($contract['cancelledAt'] ? 'Đã huỷ' : (new DateTime() < new DateTime($contract['endDate']) ? "Còn hạn" : "Hết hạn")) . '</span></h1>';
                            ...
                            echo        '<a href="contracts_add.php?MSV=' . $MSV . '&name=' . urlencode($name) . '&birthday=' . urldecode($birthday) . '&sex=' . urldecode($sex). '&phoneNumber='. urldecode($phoneNumber) . '&class='. urldecode($class). '&image='. $image . '&roomId='. $roomId . '&studentId='. $studentId .'&haveRoomId='. '"  class="p-3 bg-slate-600 text-white w-full">Gia hạn hợp đồng</a>';
                            echo    '</div>';
                        }
                        ...
                        echo        "<a target='_blank' href='contracts_print.php?ma={$contract['id']}' class='block text-center bg-green-600 p-2 text-white w-full'>In hợp đồng</a>";
                        echo    '</div>';
                        echo  '</form>';
                    }
                }
