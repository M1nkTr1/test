    ...
    if(isset($_POST['submit'])){
        ...
        if(!isset($_FILES["image"])||$_POST['name']==''||$_POST['MSV']==''||$_POST['birthday']==''||$_POST['sex']==''||$_POST['phoneNumber']==''||$_POST['class']=='')
            echo "<script>alert('Phai nhap thong tin')</script>";  
                ...
                $sql1="SELECT* FROM students where MSV='$MSV'";
                $bt=mysqli_query($con,$sql1);
                if(mysqli_num_rows($bt)>0){
                    echo"<script>alert('Trung ma sinh vien')</script>";
                }
...
<body>
    <div class="flex">
        ...
        ?>
        <div class="flex-1 p-4 flex flex-col max-h-screen overflow-scroll">
